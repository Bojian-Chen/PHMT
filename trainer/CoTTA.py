...
def CoTTA(args, teacher_backbone, teacher_classifier, student_backbone, student_classifier, train_loader, test_loader, backbone_optimizer, classifier_optimizer, backbone_scheduler, classifier_scheduler):
    ...
    # Different: We use last model instead of the source model as the initial state for the long term adaptation
            ...
            for nm, m in student_classifier.named_modules():
                for npp, p in m.named_parameters():
                    if npp in ['weight', 'bias'] and p.requires_grad:
                        mask = (torch.rand(p.shape) < rst).float().cuda()
                        with torch.no_grad():
                            p.data = initial_state_classifier[f"{nm}.{npp}"] * mask + p * (1. - mask)
