...
class RandMix(nn.Module):
    def __init__(self, noise_lv):
        ...
        self.adain_1 = AdaIN2d(zdim, 1)
        self.adain_2 = AdaIN2d(zdim, 1)
        self.adain_3 = AdaIN2d(zdim, 1)
        self.adain_4 = AdaIN2d(zdim, 1)

    def forward(self, x, estimation=False, ratio=0):
        data = x
        
        spatial1 = nn.Conv2d(1, 1, 5).cuda()
        spatial_up1 = nn.ConvTranspose2d(1, 1, 5).cuda()

        spatial2 = nn.Conv2d(1, 1, 9).cuda()
        spatial_up2 = nn.ConvTranspose2d(1, 1, 9).cuda()

        spatial3 = nn.Conv2d(1, 1, 13).cuda()
        spatial_up3 = nn.ConvTranspose2d(1, 1, 13).cuda()

        spatial4 = nn.Conv2d(1, 1, 17).cuda()
        spatial_up4 = nn.ConvTranspose2d(1, 1, 17).cuda()

        color = nn.Conv2d(1, 1, 1).cuda()
...
# print(x[0])
# data_aug = RandMix(1).cuda()
# output = data_aug(x.cuda(),ratio=0.5)
# print(output[0])