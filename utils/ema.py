...
def moving_weight(current_epoch, total_epochs, weight_begin=0.9, weight_end=0.99):
    tao = weight_end - (weight_end - weight_begin) * (math.cos(math.pi * min(1, current_epoch / total_epochs)) + 1) / 2
    return tao


def exponential_moving_average(teacher, student, current_epoch, total_epochs, tao_begin=0.9, tao_end=0.99):
